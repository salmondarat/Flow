...
export default function AddonsPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const [addonsResponse, servicesResponse] = await Promise.all([
        fetch(
          `/api/admin/addons${filterServiceId !== "all" ? `?serviceTypeId=${filterServiceId}` : ""}`
        ),
        fetch("/api/admin/services"),
      ]);

      if (!addonsResponse.ok || !servicesResponse.ok) {
        throw new Error("Failed to fetch data");
      }

      const { addons: addonsData } = await addonsResponse.json();
