/**
 * Script to apply the business_settings migration
 * Run with: npx tsx scripts/apply-business-settings-migration.ts
 */
...
-- Create policies
CREATE POLICY IF NOT EXISTS "Allow admin to read business settings"
...
CREATE POLICY IF NOT EXISTS "Allow admin to update business settings"
...
CREATE POLICY IF NOT EXISTS "Allow public to read basic business settings"
ON business_settings FOR SELECT
TO anon
USING (true);
